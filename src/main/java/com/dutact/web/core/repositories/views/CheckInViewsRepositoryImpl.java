        ...
        var query = """
                ...
                FROM (
                    ...
                    FROM EventRegistration registration
                        ...
                        LEFT JOIN EventCheckIn event_check_in ON registration.student.id = event_check_in.student.id
                        LEFT JOIN EventCheckInCode event_check_in_code ON event_check_in.checkInCode.id = event_check_in_code.id
                    WHERE (:searchQuery IS NULL OR student.fullName LIKE :searchQuery)
                        AND registration.event.id = :eventId
                        AND (event_check_in_code.id IS NULL OR event_check_in_code.event.id = :eventId)
                    GROUP BY student.id,
                             student.fullName,
                             student.avatar,
